...
module CargoRulesHelper
  ...
  def run_tf56_rule
    ...
    post_process_raw_cargo(@all_raw_cargo)
  end
  ...
  def run_56135_cargo_rule
   ...
   @mechanize.get("http://www.56135.com/56135/trade/tradeindex///#{i}.html") do |page|
    page.parser.css("div.info_show").each do |entrycontainer|
      ...
      cargo[:fcity_code]=CityTree.get_code_from_name(from_name)
      cargo[:tcity_code]=CityTree.get_code_from_name(to_name)
       cargo[:line]=cargo[:fcity_code]||""+"#"+cargo[:tcity_code]||""
  ...
  def post_process_raw_cargo(all_raw_cargo)
  #  Cargo.delete_all
